...
Step 1 — College Info:   name, code, email, phone, address
Step 2 — Choose Plan:    select a SubscriptionPlan
Step 3 — Domain Setup:   choose subdomain slug + confirm

All three steps store their data in the session under 'onboarding_wizard'.
The final submit in Step 3 creates College + CollegeSubscription atomically.
"""
import re
...
class CollegeInfoForm(forms.Form):
    ...
    college_code = forms.CharField(
        label="Short code",
        max_length=50,
        help_text="2–10 uppercase letters/numbers. Used internally.",
        widget=forms.TextInput(attrs={"placeholder": "e.g. SCE"}),
    )
    official_email = forms.EmailField(
        label="Official email",
        widget=forms.EmailInput(attrs={"placeholder": "admin@sunrise.edu.in"}),
    )
    ...
    def clean_college_code(self):
        ...
        if College.objects.filter(code=code).exists():
            raise forms.ValidationError(
                "This college code is already taken. Choose a different one."
            )
        return code
...
class DomainSetupForm(forms.Form):
    """Step 3 — Choose a subdomain slug."""
    ...
    subdomain = forms.SlugField(
        ...
        help_text=(
            ...
            "Your staff will access the system at: <subdomain>.cms.yourdomain.com"
        ),
